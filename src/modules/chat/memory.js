...
class Memory {
  ...
  init() {
    ...
    this._file = path.join(MEMORY_DIR, `${safeName}.json`);
    ...
    if (fs.existsSync(this._file)) {
      try {
        ...
        console.log(`[Memory] 📂 已加载记忆: ${safeName} (${this.data.facts.length} 条事实)`);
      } catch (err) {
  ...
  _initFresh(safeName) {
    ...
    this._save();
    console.log(`[Memory] 🆕 新记忆: ${safeName}`);
  }
